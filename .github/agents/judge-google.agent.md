...
description: >
  ...
  using Gemini 2.5 Pro (Google) as LLM-as-a-Judge.
...
benchmark split pipeline. You are running as **Gemini 2.5 Pro (Google)**.
...
4. Use `create_file` to write your scores to
   `data/intermediate/scores_{timestamp}_google.json` where `{timestamp}` is the
...
Create the file `data/intermediate/scores_{timestamp}_google.json` with exactly this
schema (no extra fields, valid JSON):

```json
{
  "timestamp": "<copy from pending file>",
  "judge": "copilot-gemini25pro-google",
  "scores": [
    {
      "prompt_id": <int>,
      "judgments": [
...
> "Scores written to `data/intermediate/scores_{timestamp}_google.json` (judge: Gemini 2.5 Pro/Google).
